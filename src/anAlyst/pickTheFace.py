...
def save_face(actor,ext,face_cascade) :
    try :
        save_img_dir =f"./public/image/{actor}{ext}"
        new_save_img_dir = "./public/image"
        print(new_save_img_dir+"/" +actor+ "_face"+ext)
        ...
        for (x,y,w,h) in faces:
            ...
            try:
                ...
                print(e)
            _, im_write_korean = cv2.imencode(ext, actor_face)
            im_write_korean.tofile(f"{new_save_img_dir}/{actor}_face{ext}")
    except :
        pass
